...
public hook_say( id )
{
	...
	{	
		for (Size = 0; Size < ArraySize(g_Stroke); Size++)
		{
			...
			if (containi(UserMessage, Stroke) != -1)
			{
				...
				{
					new szBuffer[ 64 ];
					formatex( szBuffer, 63, " (  [AMXXUCS] %s: %s  )", szUserName, UserMessage );
	...
	{		
		...
		{
			new szBuffer[ 64 ];
			formatex( szBuffer, 63, " (  [AMXXBLUE] %s: %s  )", szUserName, UserMessage );
...
public DoThePunish( id )
{
	...
	switch( get_pcvar_num( g_iPunishType ) )
	{
		...
		case 1: server_cmd( "amx_restrict_ip %s", szName);
	}
